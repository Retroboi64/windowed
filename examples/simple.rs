//! Simple example: open a window and print events to stdout.
//!
...
fn main() -> windowed::Result<()> {
    let config = WindowConfig::new("windowed — simple example")
        .size(800, 600)
        .resizable(true);
    ...
    window.run(|event| {
        match &event {
            ...
            Event::Resized { width, height } => println!("Resized → {width}×{height}"),
            Event::KeyDown(key) => println!("KeyDown  {key:?}"),
            Event::KeyUp(key) => println!("KeyUp    {key:?}"),
            Event::CharInput(ch) => println!("Char     {ch:?}"),
            Event::MouseDown { button, x, y } => println!("MouseDown {button:?} at ({x}, {y})"),
            Event::MouseUp { button, x, y } => println!("MouseUp   {button:?} at ({x}, {y})"),
            Event::MouseWheel { delta } => println!("Wheel    {delta:+}"),
